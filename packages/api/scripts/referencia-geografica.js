...
function toEnumKey(name, code, used) {
    const base = name
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .replace(/[^a-zA-Z0-9]+/g, ' ')
...
function buildEnumFile(params) {
    ...
    lines.push('} as const;', '', `export type ${params.enumType} = (typeof ${params.enumName})[keyof typeof ${params.enumName}];`, '', `export const ${params.descName} = {`);
    for (const entry of params.entries) {
        lines.push(`  [${params.enumName}.${entry.key}]: ${toTSString(entry.name)},`);
    }
    lines.push('} as const;', '', `export type ${params.descType} = (typeof ${params.descName})[keyof typeof ${params.descName}];`, '');
    return lines.join('\n');
}
const workbook = new ExcelJS.Workbook();
await workbook.xlsx.readFile(resolve(process.cwd(), 'scripts/data/CÓDIGO DE REFERENCIA GEOGRAFICA_NOVIEMBRE_2025.xlsx'));
const worksheet = workbook.worksheets[0];
if (!worksheet)
    throw new Error('No se encontro la primera hoja del Excel');
...
worksheet.eachRow((row, rowNumber) => {
    ...
    if (depCode && depName)
        departamentos.set(depCode, depName);
    if (distCode && distName)
        distritos.set(distCode, distName);
    if (cityCode && cityName)
        ciudades.set(cityCode, cityName);
});
...
const departamentosFile = buildEnumFile({
    ...
    entries: toEntries(new Map([...departamentos.entries()].sort((a, b) => Number(a[0]) - Number(b[0]))))
});
