...
const WSDLS: Record<string, string> = {
...
const XSD_BASE: Record<string, string> = {
...
async function httpsGetWithBackoff(url: string): Promise<string> {
  const maxAttempts = 5;
  let delay = 500;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    console.warn(
      `  ⚠ empty response for ${url} (attempt ${attempt}/${maxAttempts}, retrying in ${delay}ms)`
    );
...
function extractXsdLocations(content: string): string[] {
  const matches = content.matchAll(/schemaLocation=["']([^"']+\.xsd)["']/g);
  return [...matches].map((m) => m[1]!);
}

async function fetchWithDeps(baseUrl: string, filename: string, outDir: string) {
...
console.log('\nFixing type imports...');
execSync(
  `pnpm eslint src/gen/soap/ --fix --rule '{"@typescript-eslint/consistent-type-imports": ["error", {"prefer": "type-imports"}], "@typescript-eslint/consistent-type-exports": ["error", {"fixMixedExportsWithInlineTypeSpecifier": true}]}'`,
  { stdio: 'inherit' }
);
